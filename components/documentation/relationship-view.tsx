...
export const RelationshipDocView = ({ table }: { table: any }) => {
  ...
  const relationships =
    allEndpoints.filter((ep: any) => ep.tableName === table.name) || [];
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {uniqueRefs.map((ref: any, idx: number) => {
          ...
          return (
            <div
              key={idx}
              className="p-4 rounded-lg border border-border bg-sidebar hover:bg-accent/50 transition-colors cursor-pointer group"
              onClick={() => targetTableId && setSelectedTableId(targetTableId)}
            >
              <div className="flex items-center justify-between">
                <div className="text-sm">
                  <span className="font-mono font-semibold text-foreground group-hover:text-blue-500 transition-colors">
                    {thisEp.fieldNames[0]}
                  </span>
