      ...
      <div className="rounded-md border border-border overflow-hidden">
        <table className="w-full text-sm text-left">
          <thead className="bg-sidebar border-b border-border">
            <tr>
              <th className="px-4 py-3 font-medium text-muted-foreground w-8 text-center">
                #
              </th>
          ...
          <tbody className="divide-y divide-border">
            {enumItem.values.map((v: any, idx: number) => {
              const valueNote =
                v.note?.value ?? (v.note as any as string) ?? null;
              return (
                <tr
                  key={idx}
                  className="hover:bg-accent/50 transition-colors bg-background"
                >
                  <td className="px-4 py-3 text-center text-xs text-muted-foreground/50 tabular-nums">
                    {idx + 1}
                  </td>
