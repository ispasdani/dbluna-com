...
const LINKS: SceneLinkDef[] = [
  {
    // users.id → orders.user_id
    d: route(SCENE[0].x + TABLE_W, rowY(SCENE[0], 0), SCENE[1].x, rowY(SCENE[1], 1), 220),
    oneAt: [SCENE[0].x + TABLE_W + 8, rowY(SCENE[0], 0)],
    manyAt: [SCENE[1].x, rowY(SCENE[1], 1)],
    manyDir: 1,
  },
  {
    // orders.id → order_items.order_id
    d: route(SCENE[1].x + TABLE_W, rowY(SCENE[1], 0), SCENE[3].x, rowY(SCENE[3], 1), 420),
    oneAt: [SCENE[1].x + TABLE_W + 8, rowY(SCENE[1], 0)],
  ...
  },
  {
    ...
    d: `M${SCENE[2].x + TABLE_W} ${rowY(SCENE[2], 0)}H${612}Q${620} ${rowY(SCENE[2], 0)} ${620} ${rowY(SCENE[2], 0) + 8}V${rowY(SCENE[3], 2) - 8}Q${620} ${rowY(SCENE[3], 2)} ${612} ${rowY(SCENE[3], 2)}H${SCENE[3].x + TABLE_W}`,
    oneAt: [SCENE[2].x + TABLE_W + 8, rowY(SCENE[2], 0)],
    manyAt: [SCENE[3].x + TABLE_W, rowY(SCENE[3], 2)],
    manyDir: -1,
  },
];
    ...
    <div className="relative flex h-full w-full items-center justify-center">
      <svg viewBox="0 0 640 330" className="-mt-8 h-full max-h-66 w-full max-w-128 font-sans" aria-hidden>
        ...
        {LINKS.map((l, i) => (
          ...
          <SceneLink key={i} link={l} lit={i === 0} delay={0.7 + i * 0.15} />
        ))}
        ...
        <SceneRowHighlight t={SCENE[0]} row={0} delay={1.1} />
        <SceneRowHighlight t={SCENE[1]} row={1} delay={1.1} />
      </svg>
